...
def book_ticket(cur, ticket_id):
    cur.execute("UPDATE Tickets SET status = 'забронирован' WHERE id = %s", (ticket_id,))

@_add_cursor_wrapper
def cancel_booking(cur, ticket_id):
    cur.execute("UPDATE Tickets SET status = 'доступен' WHERE id = %s", (ticket_id,))

@_add_cursor_wrapper
def check_ticket_availability(cur, event_id):
    cur.execute("SELECT COUNT(*) FROM Tickets WHERE event_id = %s AND status = 'доступен'", (event_id,))
    return cur.fechone()
